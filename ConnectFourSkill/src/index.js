...
var endpoint = 'http://localhost:8080/AIGamesRestService/rest/c4';
            	...
            	sendCommand(session, getSessionGameId(session), "doAIMove", "", "", function callbackFunc(result2) {
                    ...
                    if ((result2.code === "S_AI_PLAYER_WINS") || (result2.code === "S_AI_DRAW")) {
                    	sendCommand(session, getSessionGameId(session), "closeGame", "", "", function callbackFunc(result3) {
	                    	clearSessionData(session);
	                    	speech.goodbye(intent.name, result2.code, response, result2.move.slot);
                    	});
                    }
                    else {
                    	console.log("### "+intent.name +"/"+ result2.code +"/"+ response +"/"+ result2.move.slot);
                    	speech.respond(intent.name, result2.code, response, result2.move.slot);
                    }
            	});
            }
